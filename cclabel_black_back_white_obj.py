...
#
# This code assumes that the input image has a black background and white objects, on which the algorithm will run on
...
# To color the connected component label in shades of gray to white by incrementing the shade value after every label
shade = 5
...
def run(img):
    ...
    for (x, y) in labels:
        ...
        if component not in colors: 
            colors[component] = (shade, shade, shade)
            shade = shade + 40
            # Use this if you want to color image using random RGB colors
